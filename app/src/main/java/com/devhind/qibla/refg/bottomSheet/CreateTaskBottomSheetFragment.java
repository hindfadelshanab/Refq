...
import com.devhind.qibla.refg.Older.Fragment.OldProfileFragment;
...
//    EditText addTaskDescription;
    ...
    public void setTaskId(int taskId, boolean isEdit, setRefreshListener setRefreshListener, OldProfileFragment activity) {
        this.taskId = taskId;
        this.isEdit = isEdit;
...
//            Toast.makeText(activity.getContext(), "Please enter a valid title", Toast.LENGTH_SHORT).show();
//            return false;
//        }
//        else if(addTaskDescription.getText().toString().equalsIgnoreCase("")) {
//            Toast.makeText(activity.getContext(), "Please enter a valid description", Toast.LENGTH_SHORT).show();
//            return false;
//        }
         if(taskDate.getText().toString().equalsIgnoreCase("")) {
            Toast.makeText(activity.getContext(), "Please enter date", Toast.LENGTH_SHORT).show();
            return false;
        }
        else if(taskTime.getText().toString().equalsIgnoreCase("")) {
            Toast.makeText(activity.getContext(), "Please enter time", Toast.LENGTH_SHORT).show();
            return false;
        }
        else if(taskEvent.getText().toString().equalsIgnoreCase("")) {
            Toast.makeText(activity.getContext(), "Please enter an event", Toast.LENGTH_SHORT).show();
            return false;
        }
        else {
            return true;
    ...
    public void createAnAlarm() {
        try {
            ...
            Intent alarmIntent = new Intent(activity.getContext(), AlarmBroadcastReceiver.class);
            alarmIntent.putExtra("TITLE", taskEvent.getText().toString());
          //  alarmIntent.putExtra("DESC", addTaskDescription.getText().toString());
            ...
            PendingIntent pendingIntent = PendingIntent.getBroadcast(activity.getActivity(),count, alarmIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                count ++;
                    ...
                    PendingIntent intent = PendingIntent.getBroadcast(activity.getActivity(), count, alarmIntent, 0);
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
